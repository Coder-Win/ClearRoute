...
def main():
    ...
    df_struct = pd.read_csv(processed_dir / "us_accidents_phase0_processed.csv")
    ...
    explicit_exclude = {
        ...
        # Target Variables & Derivative Leaks
        'reported_duration_minutes', 'valid_initial_duration', 'duration_requires_review',
    ...
    df_struct_only = df_merged[['ID', 'reported_duration_minutes'] + struct_cols]
    
    # Dataset 2: Structured + Semantic (Target + Tabular + LLM Flags, no embeddings, no gates)
    df_struct_sem = df_merged[['ID', 'reported_duration_minutes'] + struct_cols + sem_cols]
    ...
    # Save all five datasets
    df_struct_only.to_csv(processed_dir / "us_accidents_struct_only.csv", index=False)
    df_struct_sem.to_csv(processed_dir / "us_accidents_struct_sem.csv", index=False)
    df_struct_emb.to_csv(processed_dir / "us_accidents_struct_emb.csv", index=False)
    df_dumb.to_csv(processed_dir / "us_accidents_dumb_join.csv", index=False)
    df_fused.to_csv(processed_dir / "us_accidents_fused_multimodal.csv", index=False)
    
    print("\n[SUCCESS] All 5 Ablation Datasets safely sanitized and saved.")
